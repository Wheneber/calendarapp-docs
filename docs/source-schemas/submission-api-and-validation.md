...
- Add `guid` or `id` mapping when the feed exposes a stable source identifier.
- Add `link` or `url` mapping when the feed exposes event detail pages.
...
- There is no dedicated RSS schema filter DSL today.
- Practical filtering comes from three places only: picking the right upstream feed URL, using any upstream category/date/event-only query options the publisher already exposes, and required-field validation gates.
- Do not invent unsupported schema fields such as `filters`, `filterRules`, include/exclude keyword lists, or cancellation filters.
...
- If detail enrichment is required, `url` or `link` becomes operationally required even though it is not part of `requiredFields`.
